...
module.exports = async (req, res)=> {
  if(req.query.generate?.toLowerCase() == "true" && !req.query.genre){
    ...
    const WallpapersArray = fs.readdirSync(path.resolve(__dirname + `/../../Assets/Images/Wallpaper/${genre}`))
    const wallpaperpick = Math.floor(Math.random() * (WallpapersArray.length - 1))
    const wallpaper = WallpapersArray[wallpaperpick]
    const wallpaperpath = path.resolve(__dirname + `/../../Assets/Images/Wallpaper/${genre}/${wallpaper}`)
    res.status = 200
    ...
    if(!fs.existsSync(path.resolve(__dirname + `/../../Assets/Images/Wallpaper/${genreChoice}`))){
      res.status = 404
      return res.json({
        Status: 404,
    ...
    const wallpaperGenreArray = fs.readdirSync(path.resolve(__dirname + `/../../Assets/Images/Wallpaper/${genreChoice}`))
    ...
    const wallpaperGenrePath = path.resolve(__dirname + `/../../Assets/Images/Wallpaper/${genreChoice}/${wallpaperGenre}`)
    res.status = 200
